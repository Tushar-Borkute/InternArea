  ...
  useEffect(() => {
    const syncPhoto = () => {
      if (currentUser?.email) {
        const stored = localStorage.getItem(`candidate_photo_${currentUser.email.toLowerCase()}`);
        setUserPhoto(currentUser.photoURL || stored || "");
      } else {
        setUserPhoto("");
      }
    };
    ...
    return () => {
      ...
      window.removeEventListener("profilePhotoUpdated", syncPhoto);
    };
  }, [currentUser]);
            ...
            {currentUser ? (
              <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                <Link to="/profile" className="profile-pill-btn">
                  {userPhoto ? (
                    <img src={userPhoto} alt="Avatar" className="nav-avatar-img" />
                  ) : (
