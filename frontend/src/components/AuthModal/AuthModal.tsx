...
import { X, Mail, Lock, User, LogIn, Phone, GraduationCap, ArrowLeft, KeyRound, Eye, EyeOff, ShieldCheck } from "lucide-react";
...
const AuthModal = ({ isOpen, onClose, initialTab = "login" }: AuthModalProps) => {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (deviceInfo.deviceType === "mobile" || deviceInfo.os === "Android" || deviceInfo.os === "iOS") {
        const hour = new Date().getHours();
        if (hour < 10 || hour >= 13) {
          try {
            await axios.post(`${API_BASE_URL}/api/auth/record-login`, {
              email: "google_user@internarea.com",
              ...deviceInfo,
            });
          } catch {}
          toast.error("Access blocked: Mobile logins are only permitted between 10:00 AM and 1:00 PM.");
          return;
        }
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Pre-login check with security rules (Chrome OTP requirement & Mobile 10 AM - 1 PM restriction)
      const recordRes = await axios.post(`${API_BASE_URL}/api/auth/record-login`, {
        email,
        ...deviceInfo,
      });

      if (recordRes.data.requireOtp) {
        toast.info(recordRes.data.message || "OTP sent to your registered email for Google Chrome security verification.");
        setIsOtpStep(true);
        return;
      }
      ...
      if (err.response?.data?.blocked) {
        toast.error(err.response.data.message || "Access blocked: Mobile logins are only permitted between 10:00 AM and 1:00 PM.");
        return;
      }

      let errText = err.response?.data?.message || err.message || "Authentication failed";
        ...
        {isOtpStep ? (
          <div className="auth-body" style={{ padding: "32px 24px" }}>
            ...
            <div className="reset-header">
              <div className="reset-icon-circle" style={{ backgroundColor: "#e0f2fe" }}>
                <ShieldCheck size={28} color="#0284c7" />
              </div>
              <h2>Chrome Security Verification</h2>
              <p style={{ fontSize: "14px", color: "#64748b", marginTop: "4px" }}>
                Google Chrome login detected. Please enter the 6-digit OTP sent to <strong>{email}</strong>.
              </p>
            </div>

            <form onSubmit={verifyOtpAndLogin} className="auth-form" style={{ marginTop: "20px" }}>
              <div className="auth-input-group">
                <label>Enter 6-Digit OTP</label>
                <div className="input-wrapper">
                  <KeyRound size={18} color="#0ea5e9" />
                  <input
                    type="text"
                    maxLength={6}
                    placeholder="123456"
